...
def execute_aws_cli_ec2_description(instanceid):
    command_str = "aws ec2 describe-instances --instance-ids " + instanceid + " --query 'Reservations[*].Instances[*].{Name:Tags[?Key==`Name`]|[0].Value, Owner:Tags[?Key==`owner`]|[0].Value, Created_By:Tags[?Key==`created-by`]|[0].Value, EC2Launchtemplate:Tags[?Key==`aws:ec2launchtemplate:id`]|[0].Value,Instance:InstanceId,LaunchTime:LaunchTime,PrivateIpAddress:PrivateIpAddress}' --output table >>/Users/ellvenyao/Desktop/EC2_Risk.txt"
    print(command_str)
...
def main():
    ...
    EC2InstanceList = ["i-00d8a6c1ef4b1ca02","i-071bff4c405148481","i-0982980cc32199a1d","i-0b7ffd456fea8eba2","i-0d15efa4eebad967c","i-0fa9af89d447af8ac","i-05fbd56028d000850","i-0bfa827425e728c12","i-0cff1fa3543769c84","i-00d8a6c1ef4b1ca02","i-05fbd56028d000850","i-0bfa827425e728c12","i-0cff1fa3543769c84"]
